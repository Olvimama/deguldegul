...
- 아래 지식을 갖춘 사람은 이 문서를 이해할 수 있습니다.
  - HTTP/HTTPS
  - OAuth 2.0
...
>   - 제휴사 정보 등록 완료 후 client_id, client_secret를 전달드립니다.
> 2. **네트워크 연동 방식에 있어 "전용선" 혹은 "VPN"을 사용하실 경우 "네트워크 가이드" 문서를 먼저 검토하셔야 합니다.**
>   - 기본으로 제공하는 문서가 아니므로 별도로 요청해주시기 바랍니다.
...
본 섹션에서는 상기 플로우에 대한 상세 내용을 설명합니다.

#### 2.3.1. 제휴사 인증 및 사용자 식별

> {회사명} 플랫폼은 제휴사 인증을 위해 OAuth 2.0을 사용합니다.
>
> * 본 장은 OAuth 2.0을 처음 접하는 분을 위해 자세하게 기술하고 있습니다.
> * OAuth 2.0에 대한 경험이 있는 분은 다음 장으로 넘어가거나, 가볍게 읽고 넘어가셔도 좋습니다.
>
> - OAuth 2.0에 대한 보다 자세한 내용이 필요하다면 **Appendix A. OAuth 2.0에 대하여** 항목을 참고해주시기 바랍니다.

![](./img/seq_oauth_via_bridge.png)

*1. 서비스 진입 / 2. 브릿지 노출 후 웹토큰 요청*
  - 사용자가 서비스에 진입하면 브릿지 화면을 만납니다.
  - 이 때 {회사명} 내부적으로 웹 토큰(쿠키) 발급 과정을 거칩니다.
  - 이후 서비스 화면으로 이동합니다.

*3. Authentication(인증) 요청*
  - OAuth 인증 프로세스 첫 단계로, `3.1. 인증 코드 요청`에 해당합니다.
  - 제휴사 ID(client_id)로 제휴사를 식별하고, 브릿지 페이지에서 생성된 {회사명} 웹 토큰으로 사용자의 {회사명} 계정을 식별합니다.
    - 필요 시 사용자로부터 {회사명} OAuth 인증을 위한 약관(공통 약관) 동의를 받습니다.
    - **공통 약관을 별도로 받지 않고 서비스 자체 약관에 갈음하고 싶다면**
      - 내부 검토가 필요하므로 {회사명} 서비스 담당자에게 가이드를 요청 바랍니다.
...
- 사용자 토큰은 {회사명}에서 알 수 없으며, 제휴사에서 관리해야 합니다.
...
3. 역할: {회사명}의 보호된 자원에 접근할 때 권한이 있는지 확인
...
- 사용자 동의를 거쳐 토큰을 발급 받을 수 있는 코드를 받아옵니다.
  - 약관에 동의하지 않은 사용자에게는 {회사명} 약관 페이지를 우선 보여주며, **약관에 동의하지 않을 경우 -1000 에러**가 발생합니다.
...
- 실패
  ...
  HTTP/1.1 302 Found
  Content-Length: 0
  Location: {redirect_uri}?error=-1000
  ```
